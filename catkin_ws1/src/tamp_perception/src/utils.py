...
class Client(object):
    def __init__(self):
        ...
        try:
            ...
            raise
        # -----------------------------------------
        # connect to client2: gripper_controller/follow_joint_trajectory
        # -----------------------------------------
        try:
            self.client2 = actionlib.SimpleActionClient('gripper_controller/follow_joint_trajectory', FollowJointTrajectoryAction)
    ...
    def ungrasp(self):
        joint0 = -1.0 # the width is smaller if joint0 is bigger
        # ------------------------------
        # process joint goal
        # ------------------------------
        JOINT_NAMES = ['gripper_finger1_joint']
        joint_goal = FollowJointTrajectoryGoal()
        joint_goal.trajectory.header.stamp = rospy.Time.now()
        joint_goal.trajectory = JointTrajectory()
        joint_goal.trajectory.joint_names = JOINT_NAMES
        joint_goal.trajectory.points = [JointTrajectoryPoint(positions=[joint0], time_from_start=rospy.Duration(1.5))]
        
        # ------------------------------
        # send joint goal to client
        # ------------------------------
        self.client2.send_goal(joint_goal)
        try:
            self.client2.wait_for_result(rospy.Duration(60))
        except KeyboardInterrupt:
            self.client2.cancel_goal()
            print('error! cannot send joint goal to client2')
            raise


    def grasp(self, width):
        joint0 = width # the width is smaller if joint0 is bigger
        # ------------------------------
        # process joint goal
        # ------------------------------
        JOINT_NAMES = ['gripper_finger1_joint']
        ...
        joint_goal.trajectory = JointTrajectory()
        joint_goal.trajectory.joint_names = JOINT_NAMES
        joint_goal.trajectory.points = [JointTrajectoryPoint(positions=[joint0], time_from_start=rospy.Duration(2.0))]
